...
dados_ac <- projetos.em.andamento %>%
  dplyr::filter(!is.na(`Área de Concentração`)) %>%
  dplyr::group_by(Ano, AC = `Área de Concentração`) %>%
...
ggplot2::ggplot(
  ...
  ggalluvial::geom_flow(alpha = 0.7, width = 0.25) +
  ggalluvial::geom_stratum(width = 0.25, color = "grey30") +
  ggplot2::scale_fill_brewer(palette = "Set3") +
  ggplot2::labs(
    title = "Áreas de Concentração",
    x = "Ano",
    y = "Número de Projetos",
    fill = "Área de Concentração"
  ) +
  ...
  ggplot2::theme(
    ...
    plot.background = ggplot2::element_rect(fill = main.color, color = NA),
    panel.background = ggplot2::element_rect(fill = main.color, color = NA),
...
dados_alluvial <- projetos.em.andamento %>%
  dplyr::filter(!is.na(`Área de Concentração`),
         !is.na(`Linha de Pesquisa`),
         `Linha de Pesquisa` != "PROJETO ISOLADO") %>%
  dplyr::mutate(
    Ano = factor(Ano, levels = sort(unique(Ano))),
    AC  = `Área de Concentração`,
...
ggplot2::ggplot(
  ...
  ggplot2::scale_fill_brewer(palette = "Set3") +
  ggplot2::labs(
    title = "Linhas de Pesquisa vs. Projetos de Pesquisa",
    x = "Ano",
    y = "Número de Projetos",
    fill = "Linha de Pesquisa"
  ) +
  ggplot2::theme_minimal(base_size = 12) +
  ggplot2::theme(
    plot.title = element_text(hjust = 0.5),
...
# show sintese-coerencia-3
knitr::include_graphics("index_files/figure-html/sintese-coerencia-3.png")
cat('\n\n')
source("Scripts/fonte-sucupira.R", local = knitr::knit_global())
```
...
table.6 <- data.frame(metricas_artigos$`Valor do Item de Detalhamento`, metricas_artigos$CiteScore)
...
journals.CiteScore <- unique(table.6)
# rename columns
colnames(journals.CiteScore) <- c("metricas_artigos.journal", "metricas_artigos.CiteScore")
top <- min((as.numeric(journals.CiteScore$CiteScore)), 10)
data.journals <-
  data.frame(
    name = journals.CiteScore$metricas_artigos.journal[order(as.numeric(journals.CiteScore$metricas_artigos.CiteScore), decreasing = TRUE)][1:top],
    value = as.numeric(sort(as.numeric(journals.CiteScore$metricas_artigos.CiteScore), decreasing = TRUE
             )[1:top])
    )
# break vector names in 2 lines
text.2.break <- data.journals$name
...
# Reshape data for Plotly
data_long <- reshape2::melt(data.journals, value.name = "CiteScore")
data_long <- data_long[, -2]
colnames(data_long) <- c("Periódico", "CiteScore")
# set factor level for sorting
data_long$Periódico <- factor(data_long$Periódico, levels = data.journals$name)
...
plot.5 <- plot.5 %>% plotly::layout(
  title = list(
    text = paste0("Top-", top, " Periódicos (CiteScore)"),
    x = 0.5,
    xanchor = 'center',
    font = list(size = '100vw', color = 'white')
  ),
...
summ.table.producao <-
  cbind(
    ...
    formatC(
      min(as.numeric(metricas_artigos$CiteScore), na.rm = TRUE),
      format = "f",
      digits = 3,
      big.mark = "."
    ),
    formatC(
      median(as.numeric(metricas_artigos$CiteScore), na.rm = TRUE),
      format = "f",
      digits = 3,
      big.mark = "."
    ),
    formatC(
      max(as.numeric(metricas_artigos$CiteScore), na.rm = TRUE),
      format = "f",
      digits = 3,
      big.mark = "."
    )
...
if (sjmisc::is_empty(metricas_artigos$citations)) {
  ...
  # force matrix 
  table.4 <- as.matrix(table.4, ncol = ncol(table.4), nrow = nrow(table.4))
  # colum names
  colnames(table.4) <- sort(unique(metricas_artigos$"Ano da Produção"))
  # sum colums
...
# plot 6 (summary table for Altmetric breakdown)
...
table.2.sum <- sort(table.2.sum, decreasing = TRUE)
...
data <-
  as.data.frame(table(c(discentes$UF[discentes$UF == "RJ"], rep(
    "Outra UF", length(discentes$UF[discentes$UF != "RJ"])
  ))))
...
data <-
  as.data.frame(table(c(egressos$UF[egressos$UF == "RJ"], rep(
    "Outra UF", length(egressos$UF[egressos$UF != "RJ"])
  ))))
colnames(data) <- c("Regiões", "N")
...
if (length(IES.nac) > 0) {
  ...
  plot.3 <- plot.3 %>% plotly::layout(
    title = list(
      text = paste0("Top-", top, " Instituições Colaboradoras Nacionais"),
      x = 0.5,
      xanchor = 'center',
      font = list(size = '100vw', color = 'white')
    ),
...
if (length(IES.inter) > 0) {
  ...
  plot.4 <- plot.4 %>% plotly::layout(
    title = list(
      text = paste0("Top-", top, " Instituições Colaboradoras Internacionais"),
      x = 0.5,
      xanchor = 'center',
      font = list(size = '100vw', color = 'white')
    ),
...
if (length(mundo) > 0) {
  ...
  plot.5 <- plot.5 %>% plotly::layout(
    title = list(
      text = paste0("Top-", top, " Países com Colaboração"),
      x = 0.5,
      xanchor = 'center',
      font = list(size = '100vw', color = 'white')
    ),
