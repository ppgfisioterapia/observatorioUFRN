...
# get metricas_all that matches produtos ID
metricas_all_produtos <- metricas_all[metricas_all$"ID da Produção" %in% produtos$"ID da Produção", ]

# available years of publication
years <- sort(unique(na.omit(metricas_all_produtos$Ano)), decreasing = TRUE)

# Verifica se há DOIs
if (!sjmisc::is_empty(metricas_all_produtos)) {
  for (year in 1:length(years)) {
    ...
    table.with.badges(
      ...
      show.SJR = TRUE,
      show.Qualis = TRUE,
      metricas_all_produtos = metricas_all_produtos[metricas_all_produtos$"Ano" == years[year], ]
    )
  }
}
```
